...
class MyController():
    def __init__(self):
        ...
        self.state_land, self.future_state_land = 'P2_SEARCH', 'P2_SEARCH'
        ...
        self.range_land_sense_out= 0.32
        self.range_land_sense_in = 0.36
        ...
        self.min_stop_iter_count = 200

        # Path planner parameters
        self.index_current_setpoint = 0
        self.setpoints = [[3.75, 2.90], [3.75,0.10], [4.0,0.10], [4.0, 2.90], [4.25, 2.90], 
                          [4.25, 0.10], [4.50, 0.10], [4.50, 2.90], [4.75, 2.90], [4.75, 0.10]]
        self.x_offset_max = 0.04

        # Search Zone parameters
        self.parking_zone = []
        self.parking_clearance = 0.25
        self.parking_min_vel = 0.05
        self.stop_height = 0.07
    ...
    def land_search(self):
        ...
        # Condition for next state
        if ((self.range_down < self.range_land_sense_out  and self.x_drone > self.xlim_land_reg) or 
            np.linalg.norm([self.start_point[0]- self.x_drone, self.start_point[1]- self.y_drone]) < 0.1):
    ...
    def land(self):
        ...
        match self.state_land:
            ...
            case 'P2_SEARCH':
                # Drone keeps previous direction until it finds the second point of the landing zone (P2)
                ...
                elif self.state_land_search == 'SEARCH_BACKWARD':
                    self.v_x, self.v_y, self.w_z, self.z = -self.parking_speed, 0.0, 0.0, self.on_platform_height
                
                if self.range_down > self.range_land_sense_in :
                    self.parking_zone.append([self.x_drone, self.y_drone])
                    self.future_state_land = 'RECENTERING'

            case 'RECENTERING':
                # Drone is recentered between P1 and P2 in order to search for P3
                self.v_x = (self.parking_zone[0][0] + self.parking_zone[1][0])/2 - self.x_drone
                self.v_y = (self.parking_zone[0][1] + self.parking_zone[1][1])/2 - self.y_drone

                if abs(self.v_x) < self.parking_min_vel and abs(self.v_y) < self.parking_min_vel:
                    self.future_state_land = 'P3_SEARCH'

            case 'P3_SEARCH':
                # Drone takes predefined direction until it finds P3, the last point needed to find the 
                # landing zone center
            ...
            case 'PARKING':
                ...
                if self.state_land_search == 'SEARCH_LEFT' or self.state_land_search == 'SEARCH_RIGHT':
                    self.v_x = (self.parking_zone[2][0]-self.parking_clearance) - self.x_drone
                    self.v_y = (self.parking_zone[0][1] + self.parking_zone[1][1])/2 - self.y_drone
                elif self.state_land_search == 'SEARCH_FORWARD' or self.state_land_search == 'SEARCH_BACKWARD':
                    self.v_x = (self.parking_zone[0][0] + self.parking_zone[1][0])/2 - self.x_drone
                    self.v_y = (self.parking_zone[2][1]-self.parking_clearance)  - self.y_drone
            ...
            case 'STOP':
                ...
                if self.x_drone > self.xlim_start_reg:
                    ...
                    if self.stop_iter_count > self.min_stop_iter_count:
                        ...
                        self.future_state_land = 'P2_SEARCH'
                        self.parking_zone = []
                        print('STATUS - start')
        return
            ...
            case 'SEARCH_LEFT':
                ...
                if self.range_left <= self.safe_range_obs:
                    ...
                    if self.x_drone > self.xlim_start_reg:
                        ...
                        self.v_x, self.v_y, self.w_z, self.z = -self.searching_speed, 0.0, 0.0, self.cruising_height
                elif (abs(self.x_drone - x_goal) > self.x_offset_max and self.range_back > self.safe_range_obs and 
                      self.range_front > self.safe_range_obs):
